...
const GrainyGradient = forwardRef<GrainyGradientHandle, GrainyGradientProps>(({ ripples = [], onTimeUpdate, width = 800, height = 600 }, ref) => {
  ...
  useFrame((state) => {
    if (mesh.current) {
      ...
      const positions = new Float32Array(20)
      const times = new Float32Array(10)
      const rippleCount = Math.min(activeRipples.length, 10)

      for (let i = 0; i < rippleCount; i++) {
        const ripple = activeRipples[i]
        positions[i * 2] = (ripple.x / width) * 2 - 1
        positions[i * 2 + 1] = 1 - (ripple.y / height) * 2
...
export type { GrainyGradientHandle, Ripple }
